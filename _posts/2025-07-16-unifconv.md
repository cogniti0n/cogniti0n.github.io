...
If we take the bound

$$
\hat{L}(\hat{\theta}) - \hat{L}(\theta^*) \le \hat{L}(\hat{\theta}) - \inf_{\theta \in \Theta} \hat{L}(\theta)
$$

then the upper bound is called the *optimization error*. From optimization theory, we can prove that, for a specific class of 'good' loss functions, we can take this term to near zero by optimizing $\hat{\theta}$. Therefore, we neglect the treatment of this term and treat it like it doesn't exist for now. We rewrite this bound into a more convenient form.
...
Use Hoeffding's inequality with $\varepsilon = O\left(\sigma \sqrt{\log{n}}\right)$ where $\sigma^2 = \frac{1}{n}\sum_i (b_i - a_i)^2$, then with probability greater than $1-2n^{-2c}$ for some large constant $c$,

$$
\left|\frac{1}{n}\sum_i X_i - \mu\right| \le O\left(\sigma \sqrt{\log{n}}\right)
$$

Now we assume that the loss takes a value in $[0,1]$, e.g., classification tasks (this becomes true for most loss functions by applying a finite scale proportional to some constant $l_\infty$). Then  $\sigma = 1/\sqrt{n}$. Therefore with high probability,

$$
\left|\hat{L}(\theta) - L(\theta)\right|\le O\left(\frac{\sqrt{\log{n}}}{\sqrt{n}}\right) = \tilde{O}\left(\frac{1}{\sqrt{n}}\right)
$$

More specifically, we achieve the PAC bound
...
However, we can’t apply the same inequality to $\hat{\theta}$, since the value depends on the data. This is attributed to the fact that

$$
\forall \theta,\,\mathrm{Pr}\left[\left|\hat{L}(\theta)-L(\theta)\right|\le\varepsilon\right]\ge1-\delta
$$

is not the same as bounding
...
*Sketch of the proof*.  The union bound is very pessimistic in the sense that each of the $E_i$s overlap with each other a lot, for close $\theta$. Therefore, we can cover the whole parameter space with much fewer values of $\theta$.
...
Since our bound is of order $1/\sqrt{n}$, it is worse than the asymptotic bound from [before](https://cogniti0n.github.io/posts/sup_asymp/).We also see that the dimension of $\theta$ is present. This is an important factor - the dimension of the parameter space (or the complexity of the model) is directly related to generalization.
