...
In reinforcement learning, unlike supervised or unsupervised learning, the optimal choice is computed using rewards given on-line. For instance, take a game of Atari. The input is given as the game state, and the output is the 'best possible' action the agent can take. The environment is the game software, and the reward is the score that is shown on screen. Initially, the agent doesn't have any knowledge about the game rules nor the outputs. As it plays the game, it improves the actions it takes at a certain circumstance. 
